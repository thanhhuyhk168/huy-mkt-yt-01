...
def print_table(videos: list[dict], query: str, date_filter: str,
                min_views: int, max_subs: int | None, sort_by: str, lang: str | None):
    """Print formatted summary table."""
    lang_str = f" | lang={lang}" if lang else ""
    subs_str = f" | max {format_number(max_subs)} subs" if max_subs is not None else ""
    header = f'YouTube Topic Research: "{query}"'
    filters = f"Filters: min {format_number(min_views)} views{subs_str} | {date_filter} | sorted by {sort_by}{lang_str}"
...
def main():
    ...
    parser.add_argument("--date", "-d", choices=["today", "week", "month", "3months", "year"],
                        default="month", help="Date range (default: month)")
    parser.add_argument("--max-subs", type=int, help="Maximum subscriber count (optional)")
    ...
    if not raw_videos:
        ...
        print_table([], args.query, args.date, args.min_views, args.max_subs, args.sort, args.lang)
        return
    ...
    for v in raw_videos:
        ...
        # Apply max subs filter
        if args.max_subs is not None and sub_count > args.max_subs:
            continue
    ...
    # Print summary table
    print_table(enriched, args.query, args.date, args.min_views, args.max_subs, args.sort, args.lang)
